...
import { escapeForRegex } from '../core/helpers/regexp.js';
...
async function run() {
    ...
    const replacements = { fromRegexp, to, storage: FileStorage.OBJECT_STORAGE };
    {
        ...
        for (const query of queries) {
            const [row] = await sequelizeTypescript.query(query, { replacements, type: QueryTypes.SELECT });
            if (row['c'] !== 0)
                hasResults = true;
            console.log(` ${row['t']}`);
        }
...
async function askUpdateConfirmation() {
    return askConfirmation('These URLs can be updated, but please check your backups first (bugs happen). ' +
        'Can we update these URLs?');
}
//# sourceMappingURL=update-object-storage-url.js.map