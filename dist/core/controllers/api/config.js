import { ActorImageType, HttpStatusCode, UserRight } from '@peertube/peertube-models';
...
import { ModelCache } from '../../models/shared/model-cache.js';
...
import { apiRateLimiter, asyncMiddleware, authenticate, ensureUserHasRight, openapiOperationDoc, updateAvatarValidator, updateBannerValidator } from '../../middlewares/index.js';
...
configRouter.get('/custom', openapiOperationDoc({ operationId: 'getCustomConfig' }), authenticate, ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), getCustomConfig);
configRouter.put('/custom', openapiOperationDoc({ operationId: 'putCustomConfig' }), authenticate, ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), ensureConfigIsEditable, customConfigUpdateValidator, asyncMiddleware(updateCustomConfig));
configRouter.delete('/custom', openapiOperationDoc({ operationId: 'delCustomConfig' }), authenticate, ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), ensureConfigIsEditable, asyncMiddleware(deleteCustomConfig));
configRouter.post('/instance-banner/pick', authenticate, createReqFiles(['bannerfile'], MIMETYPES.IMAGE.MIMETYPE_EXT), ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), updateBannerValidator, asyncMiddleware(updateInstanceImageFactory(ActorImageType.BANNER)));
configRouter.delete('/instance-banner', authenticate, ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), asyncMiddleware(deleteInstanceImageFactory(ActorImageType.BANNER)));
configRouter.post('/instance-avatar/pick', authenticate, createReqFiles(['avatarfile'], MIMETYPES.IMAGE.MIMETYPE_EXT), ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), updateAvatarValidator, asyncMiddleware(updateInstanceImageFactory(ActorImageType.AVATAR)));
configRouter.delete('/instance-avatar', authenticate, ensureUserHasRight(UserRight.MANAGE_CONFIGURATION), asyncMiddleware(deleteInstanceImageFactory(ActorImageType.AVATAR)));
...
async function deleteCustomConfig(req, res) {
...
async function updateCustomConfig(req, res) {
    ...
    ClientHtml.invalidateCache();
    const data = customConfig();
    auditLogger.update(getAuditIdFromRes(res), new CustomConfigAuditView(data), oldCustomConfigAuditKeys);
    return res.json(data);
}
function updateInstanceImageFactory(imageType) {
...
function deleteInstanceImageFactory(imageType) {
    return async (req, res) => {
...
async function getServerActorWithUpdatedImages(imageType) {
    const serverActor = await getServerActor();
    const updatedImages = await ActorImageModel.listByActor(serverActor, imageType);
    if (imageType === ActorImageType.BANNER)
        serverActor.Banners = updatedImages;
    else
        serverActor.Avatars = updatedImages;
    return serverActor;
}
export { configRouter };
...
function convertCustomConfigBody(body) {
    ...
    function valueConverter(v) {
        if (validator.default.isNumeric(v + ''))
