...
import { asyncMiddleware, ensureCanAccessPrivateVideoHLSFiles, ensureCanAccessVideoPrivateWebVideoFiles, handleStaticError, optionalAuthenticate } from '../middlewares/index.js';
...
const privateHLSStaticMiddlewares = CONFIG.STATIC_FILES.PRIVATE_FILES_REQUIRE_AUTH === true
    ...
    : [];
staticRouter.use(STATIC_PATHS.STREAMING_PLAYLISTS.PRIVATE_HLS + ':videoUUID/:playlistName.m3u8', ...privateHLSStaticMiddlewares, asyncMiddleware(servePrivateM3U8));
staticRouter.use(STATIC_PATHS.STREAMING_PLAYLISTS.PRIVATE_HLS, ...privateHLSStaticMiddlewares, express.static(DIRECTORIES.HLS_STREAMING_PLAYLIST.PRIVATE, { fallthrough: false }), handleStaticError);
...
staticRouter.use(STATIC_PATHS.THUMBNAILS, express.static(thumbnailsPhysicalPath, { maxAge: STATIC_MAX_AGE.SERVER, fallthrough: false }), handleStaticError);
export { staticRouter };
