...
import { serveIndexHTML } from '../lib/html/client-html.js';
...
miscRouter.get('/robots.txt', apiRateLimiter, cacheRoute(ROUTE_CACHE_LIFETIME.ROBOTS), (_, res) => {
    res.type('text/plain');
    return res.send(CONFIG.INSTANCE.ROBOTS);
});
miscRouter.all('/teapot', apiRateLimiter, getCup, asyncMiddleware(serveIndexHTML));
miscRouter.get('/security.txt', apiRateLimiter, (_, res) => {
    return res.redirect(HttpStatusCode.MOVED_PERMANENTLY_301, '/.well-known/security.txt');
});
export { miscRouter };
async function generateNodeinfo(req, res) {
