...
import { isIntOrNull } from '../../helpers/custom-validators/misc.js';
...
const customConfigUpdateValidator = [
    ...
    (req, res, next) => {
        ...
        if (!checkInvalidTranscodingConfig(req.body, res))
            return;
        if (!checkInvalidSynchronizationConfig(req.body, res))
            return;
        if (!checkInvalidLiveConfig(req.body, res))
            return;
        if (!checkInvalidVideoStudioConfig(req.body, res))
            return;
        return next();
    }
];
function ensureConfigIsEditable(req, res, next) {
...
function checkInvalidSynchronizationConfig(customConfig, res) {
...
function checkInvalidVideoStudioConfig(customConfig, res) {
    if (customConfig.videoStudio.enabled === false)
        return true;
