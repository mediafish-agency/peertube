...
import { areValidActorHandles } from '../../helpers/custom-validators/activitypub/actor.js';
...
const unblockAccountByAccountValidator = [
    ...
    async (req, res, next) => {
        ...
        if (!await doesUnblockAccountExist(user.Account.id, targetAccount.id, res))
            return;
        return next();
    }
];
const unblockAccountByServerValidator = [
    ...
    async (req, res, next) => {
        ...
        const targetAccount = res.locals.account;
        if (!await doesUnblockAccountExist(serverActor.Account.id, targetAccount.id, res))
            return;
        return next();
    }
];
...
const unblockServerByAccountValidator = [
    ...
    async (req, res, next) => {
        if (areValidationErrors(req, res))
            return;
        const user = res.locals.oauth.token.User;
        if (!await doesUnblockServerExist(user.Account.id, req.params.host, res))
            return;
        return next();
    }
];
const unblockServerByServerValidator = [
    ...
    async (req, res, next) => {
        if (areValidationErrors(req, res))
            return;
        const serverActor = await getServerActor();
        if (!await doesUnblockServerExist(serverActor.Account.id, req.params.host, res))
            return;
        return next();
    }
];
...
async function doesUnblockAccountExist(accountId, targetAccountId, res) {
...
async function doesUnblockServerExist(accountId, host, res) {
