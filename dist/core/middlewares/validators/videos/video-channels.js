...
export const videoChannelsRemoveValidator = [
    async (req, res, next) => {
        if (!await checkVideoChannelIsNotTheLastOne(res.locals.videoChannel, res))
            return;
        return next();
    }
];
export const videoChannelsNameWithHostValidator = [
    ...
    async (req, res, next) => {
        if (areValidationErrors(req, res))
            return;
        if (!await doesVideoChannelNameWithHostExist(req.params.nameWithHost, res))
            return;
        return next();
    }
];
...
export const ensureChannelOwnerCanUpload = [
    async (req, res, next) => {
        ...
        if (!await checkUserQuota(user, 1, res))
            return;
        next();
    }
];
...
export const videoChannelImportVideosValidator = [
    ...
    async (req, res, next) => {
        ...
        if (body.videoChannelSyncId && !await doesVideoChannelSyncIdExist(body.videoChannelSyncId, res))
            return;
...
async function checkVideoChannelIsNotTheLastOne(videoChannel, res) {
