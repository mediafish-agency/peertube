...
async function createApplicationActor(applicationId) {
    const accountCreated = await createLocalAccountWithoutKeys({
...
async function sendVerifyUserEmail(user, isPendingEmail = false) {
    ...
    if (isPendingEmail)
        verifyEmailUrl += '&isPendingEmail=true';
...
async function sendVerifyRegistrationEmail(registration) {
...
async function getOriginalVideoFileTotalFromUser(user) {
...
async function getOriginalVideoFileTotalDailyFromUser(user) {
