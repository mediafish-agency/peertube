...
var xtb_trans_unit_1 = require("./xtb-trans-unit");
/**
...
var XtbFile = /** @class */ (function (_super) {
    ...
    XtbFile.prototype.initializeFromContent = function (xmlString, path, encoding, optionalMaster) {
        ...
        if (this._parsedDocument.getElementsByTagName('translationbundle').length !== 1) {
            throw new Error((0, util_1.format)('File "%s" seems to be no xtb file (should contain a translationbundle element)', path));
        }
        if (optionalMaster) {
            try {
                ...
                if (numberInMaster !== myNumber) {
                    this._warnings.push((0, util_1.format)('%s trans units found in master, but this file has %s. Check if it is the correct master', numberInMaster, myNumber));
                }
            }
            catch (error) {
                throw new Error((0, util_1.format)('File "%s" seems to be no xmb file. An xtb file needs xmb as master file.', optionalMaster.path));
            }
        }
        return this;
    };
    XtbFile.prototype.initializeTransUnits = function () {
        ...
        for (var i = 0; i < transUnitsInFile.length; i++) {
            ...
            if (!id) {
                this._warnings.push((0, util_1.format)('oops, msg without "id" found in master, please check file %s', this._filename));
            }
            var masterUnit = null;
            if (this._masterFile) {
    ...
    XtbFile.prototype.targetLanguage = function () {
        var translationbundleElem = dom_utilities_1.DOMUtilities.getFirstElementByTagName(this._parsedDocument, 'translationbundle');
    ...
    XtbFile.prototype.importNewTransUnit = function (foreignTransUnit, isDefaultLang, copyContent, importAfterElement) {
        ...
        var translationbundleElem = dom_utilities_1.DOMUtilities.getFirstElementByTagName(this._parsedDocument, 'translationbundle');
        if (!translationbundleElem) {
            throw new Error((0, util_1.format)('File "%s" seems to be no xtb file (should contain a translationbundle element)', this._filename));
        }
        ...
        var newTu = new xtb_trans_unit_1.XtbTransUnit(translationElement, foreignTransUnit.id, this, newMasterTu);
        var inserted = false;
        ...
        if (importAfterElement === undefined || (importAfterElement && !isAfterElementPartOfFile)) {
            ...
            var firstTranslationElement = dom_utilities_1.DOMUtilities.getFirstElementByTagName(this._parsedDocument, 'translation');
            ...
            var refUnitElement = dom_utilities_1.DOMUtilities.getElementByTagNameAndId(this._parsedDocument, 'translation', importAfterElement.id);
            if (refUnitElement) {
    ...
    XtbFile.prototype.createTranslationFileForLang = function (lang, filename, isDefaultLang, copyContent) {
        throw new Error((0, util_1.format)('File "%s", xtb files are not translatable, they are already translations', filename));
    };
    return XtbFile;
