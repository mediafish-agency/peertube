...
exports.AbstractMessageParser = void 0;
...
var AbstractMessageParser = /** @class */ (function () {
    ...
    AbstractMessageParser.prototype.addPartsOfNodeToMessage = function (node, message, includeSelf) {
        ...
        if (processChildren) {
            ...
            var isICU = !(0, util_1.isNullOrUndefined)(icuMessageText);
            if (isICU) {
                try {
    ...
    AbstractMessageParser.prototype.parseNormalizedString = function (normalizedString, sourceMessage) {
        ...
        try {
            ...
            throw new Error((0, util_1.format)('unexpected error while parsing message: "%s" (parsed "%")', error.message, normalizedString));
        }
            ...
            switch (token.type) {
                ...
                case parsed_message_tokenizer_1.END_TAG:
                    ...
                    if (openTags.length === 0 || openTags[openTags.length - 1] !== token.value.name) {
                        ...
                        throw new Error((0, util_1.format)('unexpected close tag "%s" (parsed "%s")', token.value.name, normalizedString));
                    }
                    openTags.pop();
                    break;
                ...
                case parsed_message_tokenizer_1.ICU_MESSAGE:
                    throw new Error((0, util_1.format)('<ICUMessage/> not allowed here, use parseICUMessage instead (parsed "%")', normalizedString));
                default:
                    break;
            }
        });
        if (openTags.length > 0) {
            ...
            throw new Error((0, util_1.format)('missing close tag "%s" (parsed "%s")', openTags[openTags.length - 1], normalizedString));
        }
    ...
    AbstractMessageParser.prototype.parseIdCountFromName = function (name) {
        ...
        if ((0, util_1.isNullOrUndefined)(match) || match[1] === '') {
            return 0;
        }
        else {
            var num = match[1];
