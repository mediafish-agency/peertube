...
export function areYoutubeImportTestsDisabled() {
    ...
    if (disabled)
        console.log('DISABLE_HTTP_YOUTUBE_IMPORT_TESTS env set to "true" so youtube import tests are disabled');
    return disabled;
}
...
export function areScalewayObjectStorageTestsDisabled() {
    if (areMockObjectStorageTestsDisabled())
        return true;
    const enabled = process.env.OBJECT_STORAGE_SCALEWAY_KEY_ID && process.env.OBJECT_STORAGE_SCALEWAY_ACCESS_KEY;
    if (!enabled) {
        console.log('OBJECT_STORAGE_SCALEWAY_KEY_ID and/or OBJECT_STORAGE_SCALEWAY_ACCESS_KEY are not set, so scaleway object storage tests are disabled');
        return true;
    }
    return false;
}
