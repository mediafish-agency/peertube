...
export class ObjectStorageCommand {
    ...
    getMockBucketName(name) {
        return `${this.seed}-${name}`;
    }
    async deleteMockBucket(name) {
        await makePostBodyRequest({
            url: ObjectStorageCommand.getMockEndpointHost(),
            path: '/ui/' + name + '?delete',
            expectedStatus: HttpStatusCode.TEMPORARY_REDIRECT_307
...
ObjectStorageCommand.DEFAULT_SCALEWAY_BUCKET = 'peertube-ci-test';
//# sourceMappingURL=object-storage-command.js.map