...
async function waitJobs(serversArg, options = {}) {
    ...
    if (!skipDelayed)
        states.push('delayed');
    const repeatableJobs = ['videos-views-stats', 'activitypub-cleaner'];
    let pendingRequests;
    function tasksBuilder() {
        const tasks = [];
        for (const server of servers) {
            if (process.env.DEBUG)
                console.log(`${new Date().toISOString()} - Checking ${server.url}`);
...
async function expectNoFailedTranscodingJob(server) {
